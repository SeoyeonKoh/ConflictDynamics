...
실제 LLM으로 실행하려면 [기본 설정](conf/config.yaml)의
...
고른 실행의 트랜스크립트와 판단 로그를 각각 탭으로 엽니다. 트랜스크립트는 MediaWiki 토론
...
`cli.py`는 Hydra 합성과 실행 연결을 맡습니다. `Config`, `Utterance`, `Decision`은
