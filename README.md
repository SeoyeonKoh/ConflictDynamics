...
선택합니다. ‘초기 대화와 참여자 입장’을 펼치면 실제 실행에 사용할 시드와 참여자를 확인할 수
...
- 순서 규칙, 기억 모드, 최대 틱, 난수 시드를 화면에서 바꿀 수 있습니다.
  에이전트·페르소나·시드는 선택한 프리셋, 모델 등 나머지는 `conf/config.yaml`을 따릅니다.
...
CLI에서도 `uv run --no-sync conflict-score --live runs/live/<실행명>`으로 같은 기능을 사용할 수 있습니다.
