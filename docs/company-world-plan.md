...
- [ ] 하루 페이즈: 출근 → 오전 업무 → 점심 → 오후 업무 → 퇴근 → (야근). 페이즈·캘린더·외부 충격 일정은 `loop.py` 소관. [2026-09-24] 출퇴근 구현: 하루 끝(closing 틱 뒤) 전원 로비로(`Office.leave`), 다음 날 arrival은 로비에서 시작 — 뷰어는 퇴근 시 로비에서 사라지고 출근 시 나타난다. 야근은 C-13.
...
- [ ] **A의 세션 2종** — `talk`(같은 장소 재실, `event_driven`; [개정 2026-09-21] 참여자는 Action의 `targets`에 이름 적힌 사람만 — 재실 전원이 아님. 4번째 실 API 날에 전원 참여 규칙이 에이전트를 32틱 중 15–19틱 busy로 묶었다) · `message`(DM, 아래 두 상태). 회의 턴제, 비공개 채널(뒷담화), DM 무응답 → "ignored" 레코드는 **C**.
...
- [ ] **outcome hook** — 세션 종료 시 관계·내부 상태·Task에 반영. 현재 가장 크게 빠진 연결 고리. 산정 방식은 아래. [개정 2026-09-24] 발화별 valence는 **듣는 사람의 판단**(그 발화 뒤 첫 decide)으로 — 전에는 말한 사람 자신의 valence가 듣는 사람의 관계를 움직였다. 기본 `relation_appraisal: llm`: 세션이 끝나면 참여자마다 LLM 1회로 다른 발화자 각각을 (valence, arousal, 이유 한 줄) 평가하고 그 값이 `received`가 된다. 이유는 관찰 레코드로 남는다. `listener`는 추가 호출 없는 기준선.
...
#### outcome hook 산정 — 에이전트 평가 + 규칙 [개정 2026-09-24]

세션이 끝나면 `relation(a→b)`, `grievances`, `stress`·`mood`, Task 상태를 갱신한다. **숫자 규칙은 그대로, 입력은 에이전트 자신의 평가.**

- **기본 `relation_appraisal: llm`** — 참여자마다 세션 끝 LLM 1회로 다른 발화자 각각을 자기 관점에서 평가한다: `valence`(−1~1) · `arousal` · 이유 한 줄. 평가가 `received`가 되고, 이유는 관찰 레코드로 남는다(나중에 recall).
- **`listener`** — 추가 호출 없는 기준선: 나를 향한 발화마다, 그 발화 뒤 **내가** 처음 내린 판단(`decide`)의 valence·arousal.
- 구조 사실(거부 · 무시 · 편들기 · 공개)은 두 방식 모두 규칙으로 더한다 — 객관적 사건이라 평가할 필요가 없다.

결정 경위: 원안은 "LLM 판정은 쓰지 않는다 — 호출 ↑, '착하게' 평가하는 편향, 비결정적"이었다. 그런데 (1) 구현이 발화 **작성자**의 valence를 **듣는 사람**의 관계에 더하고 있었고(초조한 Blake가 Erin→Blake를 깎음, 9cf0f16에서 수정), (2) 규칙 집계는 무엇이 왜 나빴는지를 남기지 못해 수치가 에이전트의 인식을 반영하지 못했다. 사용자 결정으로 LLM 평가를 기본으로 한다. 원안이 예상한 편향은 실제로 나왔다(real-2day-b: outcome 102건 중 음수 2, 평균 +0.38, 4쌍 +1 포화) — 프롬프트에 **"0 = 평범한 업무 대화"** 기준점을 두고(감사·친절한 말투·자기 일 착수는 0, 업무를 넘는 도움만 +, 압박·책임 전가·무시·약속 불이행은 −) `w_v`를 0.2 → 0.1로 내려 보정했다(b65401f: 평범한 진행 확인 +0.40 → 0.00). 비결정성과 호출 비용(하루 약 50~100회, 수만 토큰)은 감수한다. `listener`는 비교 실험 기준선으로 유지.

```text
relation(a→b) += w_v · valence(a가 평가한 b)   # llm: 세션 평가 / listener: b 발화 뒤 a의 판단 평균
               − w_s · [b가 a의 요청 거부 or 무시]
               × (public ? 1.5 : 1)                             # 공개 석상 배율
grievance     += (a의 요청 거부 or 공개 반박) and 미해소 → reflection id 참조
stress(a)     += w_a · Σ arousal(같은 출처) + w_s · [요청 거부]     # 매 틱 −ρ 회복
